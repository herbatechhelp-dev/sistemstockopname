<x-layouts.app title="{{ isset($user) ? 'Edit' : 'Tambah' }} User" header="{{ isset($user) ? 'Edit' : 'Tambah' }} User">
    <div class="max-w-2xl">
        <form method="POST" action="{{ isset($user) ? '/admin/users/' . $user->id : '/admin/users' }}" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            @csrf
            @if(isset($user)) @method('PUT') @endif
            <div class="grid grid-cols-2 gap-4">
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Nama</label><input type="text" name="name" value="{{ old('name', $user->name ?? '') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg"></div>
            ...
            <div class="flex space-x-3">
                ...
                <a href="/admin/users" class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200">Batal</a>
            </div>
        </form>
    </div>
</x-layouts.app>
