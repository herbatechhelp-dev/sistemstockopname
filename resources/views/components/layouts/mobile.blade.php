    ...
    <div class="fixed bottom-4 left-4 right-4 bg-slate-900/95 backdrop-blur-lg border border-slate-700/50 rounded-2xl shadow-xl shadow-slate-900/40 z-40 overflow-hidden">
        <div class="flex justify-around py-3 px-2">
            @if(in_array(auth()->user()->role, ['petugas_so', 'team_leader']))
                <a href="/entry" class="flex flex-col items-center py-1.5 px-3 rounded-xl transition-all duration-200 {{ request()->is('entry*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 scale-105' : 'text-slate-400 hover:text-slate-200' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                    <span class="text-[10px] mt-1 font-semibold tracking-wide">SO Input</span>
                </a>
                <a href="{{ route('my-sessions.index') }}" class="flex flex-col items-center py-1.5 px-3 rounded-xl transition-all duration-200 {{ request()->is('my-sessions*') ? 'bg-slate-700 text-white shadow-lg shadow-slate-700/30 scale-105' : 'text-slate-400 hover:text-slate-200' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                    <span class="text-[10px] mt-1 font-semibold tracking-wide">Sesi Saya</span>
                </a>
            @endif
            @if($mobileMultiple)
                <a href="{{ route('session.picker') }}" class="flex flex-col items-center py-1.5 px-3 rounded-xl transition-all duration-200 {{ request()->is('session/picker') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 scale-105' : 'text-slate-400 hover:text-slate-200' }}">
            ...
            @if(auth()->user()->role === 'team_leader')
                <a href="/verification" class="flex flex-col items-center py-1.5 px-3 rounded-xl transition-all duration-200 {{ request()->is('verification*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/30 scale-105' : 'text-slate-400 hover:text-slate-200' }}">
